...
def sharpe_ratio(
    ...
    risk_free_rate_annual: float = 0.0525,
    periods_per_year: int = 252,
) -> float:
    """Annualized Sharpe ratio from period-over-period returns of the equity
    curve. periods_per_year should match the equity curve's actual sampling
    frequency (252 for daily marks, ~252*375 for 1-min intraday marks during
    NSE trading hours, etc.) — passing the wrong value silently produces a
    wrong annualization, so this is a required, not optional, judgment call
    by the caller."""
...
def sortino_ratio(
    ...
    """Like Sharpe, but only penalizes downside volatility — often more
    appropriate for strategies with asymmetric payoff shapes (e.g. options
    buying), since it doesn't penalize large favorable moves the way Sharpe
    does by treating all volatility as equally 'risky'."""
    if len(equity_curve) < 3:
        return 0.0
    returns = equity_curve.pct_change().dropna()
    returns = returns.replace([np.inf, -np.inf], np.nan).dropna()
